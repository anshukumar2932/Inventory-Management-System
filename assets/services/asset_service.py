...
class AssetService:
    ...
    @staticmethod
    def get_scoped_asset_qs(user, include_unapproved=False):
        qs = Asset.objects.select_related("category", "location", "department", "vendor")
        if user.role == "SUPER_ADMIN":
            scoped = qs.all()
        elif user.department:
            scoped = qs.filter(department=user.department)
        else:
            scoped = qs.none()
        ...
        return scoped.filter(approval_status="APPROVED")
    ...
    def get_dashboard_stats(user):
        ...
        repair_total = r_qs.aggregate(s=Sum("repair_cost"))["s"]
