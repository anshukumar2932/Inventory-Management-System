...
const API = "http://localhost:8000/api/v1/audits"
...
export default function AuditScanner() {
    ...
    const createSession = async () => {
        ...
        const res = await fetch(`${API}/sessions/`, {
            method: "POST",
            headers: headers(),
            body: JSON.stringify({ department: parseInt(newDept), status: "OPEN" }),
        })
        if (res.ok) {
            const data = await res.json()
            setSession(data)
    ...
    const completeSession = async () => {
        ...
        setSession(null)
        const d = await fetch(`${API}/sessions/`, { headers: headers() }).then((r) => r.json())
