...
const API = `${API_BASE}/api/v1/repairs`
...
export default function Repairs() {
    ...
    const handleSubmit = async (e) => {
        ...
        const res = await (method === "PUT" ? apiPut(url, body) : apiPost(url, body))
        if (res.ok) {
    ...
    const handleDelete = async (id) => {
        ...
        await apiDelete(`${API}/repairs/${id}/`)
        fetchTickets()
    }
    ...
    const handleDocUpload = async (ticketId) => {
        ...
        fd.append("file", file)
        const res = await apiUpload(`${ASSETS_API}/documents/`, fd)
        if (res.ok) {
            const doc = await res.json()
            await apiPatch(`${API}/repairs/${ticketId}/`, { bill_documents: [doc.id] })
                                    ...
                                    <td style={tdStyle}>
                                        {t.bill_documents?.length > 0 ? (
                                            <span style={{ color: "#22c55e", fontSize: "0.8rem", cursor: "pointer" }}
                                                onClick={async () => {
                                                    const r = await apiGet(`${ASSETS_API}/documents/${t.bill_documents[0]}/download/`);
