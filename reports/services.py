...
def _executive_header(title: str, user=None):
    ...
    if user:
        ...
        if getattr(user, 'department', None):
            scope_label = user.department.department_name

    rows = [
        ['Report', title],
        ['Scope', scope_label],
        ['Generated On', date.today().strftime('%A, %B %d, %Y')],
        ['Generated By', generated_by],
    ]

    t = Table(rows, colWidths=[95, 395])
    t.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, -1), RL_CARD),
...
def _priority_alerts_table(metrics: dict, styles: dict):
    alerts = []

    if metrics['missing_assets']:
        alerts.append([
            'Critical',
            f"{metrics['missing_assets']} assets are marked missing and need immediate investigation.",
        ])

    if metrics['expired_services']:
        alerts.append([
            'High',
            f"{metrics['expired_services']} service agreements have already expired.",
        ])

    if metrics['expiring_7']:
        alerts.append([
            'High',
            f"{metrics['expiring_7']} service agreements expire within 7 days.",
        ])

    if metrics['pending_procurements']:
        alerts.append([
            'Watch',
            f"{metrics['pending_procurements']} procurement requests are pending approval.",
        ])

    if metrics['pending_approvals']:
        alerts.append([
            'Watch',
            f"{metrics['pending_approvals']} asset approvals are waiting for review.",
        ])

    if metrics['under_repair'] and metrics['repair_rate'] >= 10:
        alerts.append([
            'Watch',
            f"Repair load is elevated at {metrics['repair_rate']}% of all tracked assets.",
        ])

    if not alerts:
        alerts.append([
            'Stable',
            'No critical operational exceptions were detected in the current reporting window.',
        ])

    data = [[
        Paragraph('<b>Priority</b>', styles['body']),
        Paragraph('<b>What Needs Attention</b>', styles['body']),
    ]]

    for priority, detail in alerts[:5]:
        color = RL_GREEN
        ...
        data.append([
            Paragraph(f'<font color="{color.hexval()}"><b>{priority}</b></font>', styles['body']),
            Paragraph(detail, styles['body']),
        ])

    t = Table(data, colWidths=[85, 405], repeatRows=1)
    t.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), RL_PANEL),
        ('BACKGROUND', (0, 1), (-1, -1), RL_CARD),
        ('BOX', (0, 0), (-1, -1), 1, RL_BORDER),
        ('INNERGRID', (0, 0), (-1, -1), 0.5, RL_BORDER),
        ('TEXTCOLOR', (0, 0), (-1, -1), RL_TEXT),
        ('VALIGN', (0, 0), (-1, -1), 'TOP'),
...
def _business_snapshot_table(metrics: dict, styles: dict):
    rows = [
        ['Operational Health', 'Value', 'Weekly Movement', 'Value'],
        ['Utilisation Rate', f"{metrics['utilisation_rate']}%", 'New Repairs (7d)', str(metrics['repairs_last_week'])],
        ['Compliance Score', f"{metrics['compliance_score']}%", 'New Procurements (7d)', str(metrics['proc_last_week'])],
        ['Assets Under Repair', str(metrics['under_repair']), 'Open Repair Tickets', str(metrics['open_repairs'])],
        ['Total Repair Cost', f"${metrics['total_repair_cost']:,.0f}", 'Completed Repairs', str(metrics['completed_repairs'])],
    ]

    t = Table(rows, colWidths=[140, 90, 140, 120], repeatRows=1)
    t.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), RL_PANEL),
        ('BACKGROUND', (0, 1), (-1, -1), RL_CARD),
        ('BOX', (0, 0), (-1, -1), 1, RL_BORDER),
...
def _top_expiring_services_table(metrics: dict, styles: dict):
    expiring = metrics.get('expiring_details', [])[:5]

    data = [[
        Paragraph('<b>Asset</b>', styles['body']),
        Paragraph('<b>Service</b>', styles['body']),
        Paragraph('<b>Expiry</b>', styles['body']),
        Paragraph('<b>Days Left</b>', styles['body']),
    ]]
    ...
    if not expiring:
        data.append([
            Paragraph('No assets', styles['body']),
            Paragraph('No services are expiring soon.', styles['body']),
            Paragraph('-', styles['body']),
            Paragraph('-', styles['body']),
        ])
    else:
        for item in expiring:
            exp_d = date.fromisoformat(item['end_date']) if isinstance(item['end_date'], str) else item['end_date']
            ...
            data.append([
                Paragraph(f"{item['asset__asset_code']}<br/><font color='{MUTED}'>{item['asset__asset_name'][:24]}</font>", styles['body']),
                Paragraph(item['service_type__name'], styles['body']),
                Paragraph(str(exp_d), styles['body']),
                Paragraph(f"<font color='{color.hexval()}'><b>{days_left}d</b></font>", styles['body']),
            ])

    t = Table(data, colWidths=[145, 150, 95, 90], repeatRows=1)
    t.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), RL_PANEL),
        ('BACKGROUND', (0, 1), (-1, -1), RL_CARD),
        ('BOX', (0, 0), (-1, -1), 1, RL_BORDER),
...
def generate_pdf(metrics: dict, charts: dict,
    ...
    elements.append(Paragraph(
        "Important details are surfaced first so decision-makers can act before moving into analysis and supporting tables.",
        S['sub_title'],
    ))
    elements.append(_executive_header(title, user=user))
    elements.append(Spacer(1, 12))
    elements.append(_section_divider('Top Priorities', S))
    elements.append(Spacer(1, 6))
    elements.append(_priority_alerts_table(metrics, S))
    elements.append(Spacer(1, 12))
    elements.append(_section_divider('Key Performance Indicators', S))
    elements.append(Spacer(1, 8))
    elements.append(_kpi_cards_table(metrics, S))
    elements.append(Spacer(1, 12))
    elements.append(_section_divider('Business Snapshot', S))
    elements.append(Spacer(1, 6))
    elements.append(_business_snapshot_table(metrics, S))
    elements.append(Spacer(1, 12))
    elements.append(_section_divider('Top 5 Expiring Services', S))
    elements.append(Spacer(1, 6))
    elements.append(_top_expiring_services_table(metrics, S))
    ...
    elements.append(Spacer(1, 6))
    elements.append(Paragraph(
        "The charts below show the current operating picture before the detailed registers that follow.",
        S['body'],
    ))
    elements.append(Spacer(1, 6))
    ...
    # ── PAGE 3+: COMPLIANCE & SERVICE EXPIRY ─────────────────────────────
